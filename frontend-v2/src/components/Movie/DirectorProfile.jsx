...
const DirectorProfile = ({ personId }) => {
  ...
  const fetchWikiData = async (name) => {
    try {
      ...
      // Placeholder for works and news
      setWikiData({
  ...
  if (error || !director) {
    return <Alert message={error || "Director not found"} type="error" />;
  }
  ...
  return (
    <div className="mn-face-card">
      ...
      <div className="mn-face-info">
        <Text strong>{director.name}</Text>
        <Text>Director</Text>
